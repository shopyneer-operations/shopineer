...
export interface CategorySliderWidgetConfig {
  title: string;
  metadataKey: string;
  countKey: string;
...
export const EditForm = ({
  ...
  config: CategorySliderWidgetConfig;
}) => {
  ...
  useEffect(() => {
    const existingImages = (category.metadata?.[config.metadataKey] as any[]) || [];
  ...
  }, [category.metadata?.[config.metadataKey], form, config.metadataKey]);
...
export const createCategorySliderWidget = (config: CategorySliderWidgetConfig) => {
  const CategorySliderWidget = ({ data: passedCategory }: DetailWidgetProps<AdminProductCategory>) => {
    const { data: category, mutate } = useSWR(["category", passedCategory.id], () =>
      sdk.admin.productCategory.retrieve(passedCategory.id)
    );
    const [isOpen, setIsOpen] = useState(false);

    const sliderImages = category?.product_category?.metadata?.[config.metadataKey] as any[] | undefined;
                ...
                {sliderImages?.slice(0, 6).map((item, index) => {
                  ...
                  return (
                    <div key={index} className="relative">
                      <img
                        src={imageUrl}
                        alt={`Category slider preview ${index + 1}`}
