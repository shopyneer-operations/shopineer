  ...
  const handleSubmit = form.handleSubmit(async ({ sliderImages }) => {
    try {
      ...
      const result = await sdk.admin.store.update(store.id, {
        metadata: {
          ...store.metadata,
          secondary_slider_images: allImages.map((image) => image.url),
          secondary_slider_images_count: allImages.length,
        },
      });

      // Show success toast
      toast.success("تم تحديث صور السلايدر الثانوي بنجاح", {
      ...
      toast.error("فشل تحديث صور السلايدر الثانوي", { description: error.message });
    }
  });
      ...
      <Drawer.Content>
        <FormProvider {...form}>
          <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
            <Drawer.Header>
              <Heading className="capitalize">تعديل صور السلايدر الثانوي</Heading>
            </Drawer.Header>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {currentImages.map((image, index) => (
                      <div key={index} className="relative group">
                        <img
                          src={image.url}
                          alt={`Secondary slider image ${index + 1}`}
...
const SecondarySliderUploadWidget = ({ data }: DetailWidgetProps<AdminStore>) => {
  ...
  const sliderImages = store?.metadata?.secondary_slider_images as string[] | undefined;
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <div className="flex items-center justify-between px-6 py-4">
        <Heading level="h2">صور السلايدر الثانوي للصفحة الرئيسية</Heading>
      ...
      <div className="px-6 py-4">
        {imageCount > 0 ? (
            ...
            <div className="grid grid-cols-3 gap-3">
              {sliderImages?.slice(0, 6).map((imageUrl, index) => (
                <img
                  key={index}
                  src={imageUrl}
                  alt={`Secondary slider preview ${index + 1}`}
          ...
          <p className="text-sm text-ui-fg-subtle">لا توجد صور للسلايدر الثانوي. اضغط على "تعديل" لإضافة صور.</p>
        )}
      </div>
