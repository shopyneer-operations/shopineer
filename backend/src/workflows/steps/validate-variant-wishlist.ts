...
export const validateVariantWishlistStep = createStep(
  ...
  async ({ variant_id, sales_channel_id, wishlist }: ValidateVariantWishlistStepInput, { container }) => {
    // validate whether variant is in wishlist
    const isInWishlist = wishlist.items?.some((item) => item.product_variant_id === variant_id);

    if (isInWishlist) {
      throw new MedusaError(MedusaError.Types.INVALID_DATA, "Variant is already in wishlist");
    }
    ...
    if (!variantInSalesChannel) {
      throw new MedusaError(MedusaError.Types.INVALID_DATA, "Variant is not available in the specified sales channel");
    }
  }
);
