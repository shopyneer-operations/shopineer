...
export const validateCustomerCreateWishlistStep = createStep(
  ...
  async ({ customer_id }: ValidateCustomerCreateWishlistStepInput, { container }) => {
    ...
    if (data.length) {
      throw new MedusaError(MedusaError.Types.NOT_FOUND, "Customer already has a wishlist");
    }
    ...
    if (customers.length === 0) {
      throw new MedusaError(MedusaError.Types.INVALID_DATA, "Specified customer was not found");
    }
  }
);
